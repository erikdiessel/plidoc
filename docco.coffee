...
For MathJax support use this approach:
str.replace(/aa((?:(?!aa).)*)aa/g, function(s) { return s.toUpperCase(); })
returns *str* with all characters enclosed in 'aa'-patterns uppercased
example:
>>> str = "test bbx aa byxc,y,. aa ycvxycvx, aa cyvx,my, aa vyvycx"
>>> str.replace(/aa((?:(?!aa).)*)aa/g, function(s) { return s.toUpperCase(); })
    'test AA ADGS AA fdsAAS AA adts aa'
?! is negative lookahead: no string which matches this pattern
replace can take a 
)###
...
# Escapes the characters in *str* for use in a regular expression
string_regex = (str) ->
  return new RegExp(regex_escape(str), 'g')

escape_for_markdown = (str) ->
  escaped_symbols = [ '\\', '`', '*', '_', '{', '}', '[', ']',
                      '(' , ')', '#', '+', '-', '.', '!' ]
  # escape recursively all *escaped_symbols* in the string,
  # *acc* holds the currently processed, partially escaped string
  return escaped_symbols.reduce (acc, symbol) ->
    return acc.replace(string_regex(symbol), '\\' + symbol)
  , str

# sample call:
# escape_math("*highlighed*: $$ a_n = \\sqrt{n} $$, a **nice** formula")
#
...
escape_math = (str) ->
  ...
  # escape any characters inside of mathematical expressions
  return math_tags.reduce( (acc, math_tag) ->
    return acc.replace(new RegExp( 
      # starts with the *startTag*
      '(' + regex_escape(math_tag.startTag)
      # does not include the *startTag* inside it => smallest possible match
          + '(?:(?!' + regex_escape(math_tag.startTag) + ').)*'
      # ends with *endTag*
